...
static INT8U    led_vibrator_onoff;
static INT16U	power_off_cnt = 0;
static INT16U   chrg_cnt_flg = 0;
static INT16U   chrg_record_flg = 0;
...
void ap_peripheral_prev_key2(INT16U * tick_cnt_ptr);
...
void LED_pin_init(void)
{
	...
	gpio_init_io(VIBRATOR, GPIO_OUTPUT);
	gpio_set_port_attribute(VIBRATOR, ATTRIBUTE_HIGH);
	gpio_write_io(VIBRATOR, DATA_LOW);

	//OSTimeDly(30);
	led_night_flag		= 0;
	led_red_flag		= LED_OFF;
	led_green_flag		= LED_OFF;
	led_vibrator_onoff  = LED_OFF;
	type				= LED_INIT;
...
void vibrator_turn_on(void)
{
	if(LED_ON != led_vibrator_onoff)
	{
		__msg("vibrator_turn_on\n");
		gpio_write_io(VIBRATOR, DATA_HIGH);
		led_vibrator_onoff = LED_ON;
	}
}

void vibrator_turn_off(void)
{
	if(LED_OFF != led_vibrator_onoff)
	{
		__msg("vibrator_turn_off\n");
		gpio_write_io(VIBRATOR, DATA_LOW);
		led_vibrator_onoff = LED_OFF;
	}
}
...
void ap_peripheral_key_register(INT8U type)
{
	...
	if (type == GENERAL_KEY)
	{
		...
		key_map[1].key_function = (KEYFUNC)ap_peripheral_prev_key2;
...
void ap_peripheral_key_judge(void)
{
	...
	/******************************************************
	 *
	 *When charging states, after press key, 1 minute later
	 *if not do anything, then return display charge states.
	 *
	*******************************************************/
	if((2 == usb_state_get()) && chrg_record_flg)
	{
		if(chrg_cnt_flg-- <= 0) 	// 1 minutes
		{
			INT16U			led_type;
			chrg_record_flg = 0;
	...
	/*******************************************************/
	//If not do anything, power off when 2 minitues later.
...
void ap_peripheral_pw_key_exe(INT16U * tick_cnt_ptr)
{
	INT8U i;
	INT32U led_type;
	__msg("ap_peripheral_pw_key: tick_cnt_ptr = %d, s_usbd_pin = %d\n", *tick_cnt_ptr, s_usbd_pin);
...
void ap_peripheral_prev_key2(INT16U * tick_cnt_ptr)
{
	...
	if((2 == usb_state_get())&& chrg_record_flg != 1)
	{
		*tick_cnt_ptr		= 0;
		__msg("usb_state_get(): %d\n", usb_state_get());
		return;
	}
	__msg("ap_peripheral_prev_key2: tick_cnt_ptr = %d, s_usbd_pin = %d\n", *tick_cnt_ptr, s_usbd_pin);
	if (!s_usbd_pin)
	{
...
{
	INT8U i;
	__msg("ap_peripheral_video_prev: tick_cnt_ptr = %d\n", *tick_cnt_ptr);
	if (!s_usbd_pin)
	{
		if ((ap_state_handling_storage_id_get() != NO_STORAGE) && (!pic_down_flag) && (!card_space_less_flag) &&
			 (!video_down_flag))
