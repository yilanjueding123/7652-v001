...
void ap_storage_service_init(void)
{
	...
	sys_set_timer((void *) msgQSend, (void *) StorageServiceQ, MSG_STORAGE_SERVICE_STORAGE_CHECK,
		 storage_mount_timerid, STORAGE_TIME_INTERVAL_MOUNT);
...
void ap_storage_service_freesize_check_switch(INT8U type)
{
	if (type == TRUE)
	{
		if (storage_freesize_timerid == 0xFF)
		{
			...
			sys_set_timer((void *) msgQSend, (void *) StorageServiceQ, MSG_STORAGE_SERVICE_FREESIZE_CHECK,
				 storage_freesize_timerid, STORAGE_TIME_FIRST_CHECK); //STORAGE_TIME_INTERVAL_FREESIZE
...
void ap_storage_service_free_filesize_check()
{
	...
	if (nRet >= 0)
	{
		while (1)
		{
			...
			{
				if (! (file_info.f_attrib & _A_RDONLY))
				{ //modified by wwj, stat() spent too long time
...
RE_DEL:
		...
		if (bkground_del_disble == 1)
		{
			...
			{
				sys_set_timer((void *) msgQSend, (void *) StorageServiceQ, MSG_STORAGE_SERVICE_FREESIZE_CHECK,
					 storage_freesize_timerid, STORAGE_TIME_INTERVAL_FREESIZE); //
			...
			if (disk_free_size <= 50)
			{
				...
				if (disk_free_size < CARD_FULL_SIZE_RECORD)
				{
					...
					sys_set_timer((void *) msgQSend, (void *) StorageServiceQ, MSG_STORAGE_SERVICE_FREESIZE_CHECK,
						 storage_freesize_timerid, BKGROUND_DETECT_INTERVAL);
				}
			}
			else //if (current_movie_Byte_size_get(pAviEncPara->AviPackerCur->avi_workmem)>AVI_REC_MAX_BYTE_SIZE)
			{
				...
				else if (file_size > (AVI_REC_MAX_BYTE_SIZE - BASE_SIZE))
				{
					...
					sys_set_timer((void *) msgQSend, (void *) StorageServiceQ, MSG_STORAGE_SERVICE_FREESIZE_CHECK,
						 storage_freesize_timerid, BKGROUND_DEL_INTERVAL);
				}
			}
...
void ap_storage_service_file_lock_one(void)
{
	...
	if ((avi_file_table[g_play_index >> 4] & (1 << (g_play_index & 0xF))))
	...
	switch (req_type)
	{
		case MSG_STORAGE_SERVICE_VID_REQ:
			...
			if (file_info.file_handle >= 0)
			{
				...
				file_info.file_path_addr = (INT32U)
				g_file_path;
				...
				g_txt_path[11]		= 't';
				g_txt_path[12]		= 0;
				file_info.txt_path_addr = (INT32U)
				g_txt_path;
...
void ap_storage_service_play_req(STOR_SERV_PLAYINFO * info_ptr, INT32U req_msg)
{
	...
	info_ptr->file_path_addr = (INT32U)
	g_file_path;
		...
		if (gp_strncmp((INT8S *) pdata, (INT8S *) "MOVI", 4) == 0)
		{
			...
			if (temp < 10000)
			{
				...
				if (((!g_avi_file_time) || (temp_time > g_avi_file_time)) && /*((buf_tmp.st_mode & D_RDONLY) == 0)
					*/ ((file_info.f_attrib & _A_RDONLY) == 0))
				{
					g_avi_file_time 	= temp_time;
					g_latest_avi_file_index = temp;
				}

				if (((!g_avi_file_oldest_time) || (temp_time < g_avi_file_oldest_time)) && /*((buf_tmp.st_mode & D_
					RDONLY) == 0)*/ ((file_info.f_attrib & _A_RDONLY) == 0))
				{
...
void ap_storage_service_format_req(void)
{
	...
	if (drvl2_sd_sector_number_get() <= 2097152)
	{ // Dominant  1GB card should use FAT16
