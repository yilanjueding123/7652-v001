...
extern void vibrator_turn_on(void);
extern void vibrator_turn_off(void);
...
void ap_get_ldw_display_coordinate(void)
{
	...
	ldw_dis.ldw_dis_LT_alarmP_Y =
		 divFun((LDWPar.LT_alarmP_Y * LDWPar.scaleRate_F), 10) +LDWPar.ROI.y + LDWPar.ROI_initY; // + 10;
	ldw_dis.ldw_dis_LB_alarmP_X = divFun((LDWPar.LB_alarmP_X * LDWPar.scaleRate_F), 10) +LDWPar.ROI.x;
	ldw_dis.ldw_dis_LB_alarmP_Y =
		 divFun((LDWPar.LB_alarmP_Y * LDWPar.scaleRate_F), 10) +LDWPar.ROI.y + LDWPar.ROI_initY; //+ 10;
	...
	ldw_dis.ldw_dis_RT_alarmP_Y =
		 divFun((LDWPar.RT_alarmP_Y * LDWPar.scaleRate_F), 10) +LDWPar.ROI.y + LDWPar.ROI_initY; //+ 10;
	ldw_dis.ldw_dis_RB_alarmP_X = divFun((LDWPar.RB_alarmP_X * LDWPar.scaleRate_F), 10) +LDWPar.ROI.x;
	ldw_dis.ldw_dis_RB_alarmP_Y =
		 divFun((LDWPar.RB_alarmP_Y * LDWPar.scaleRate_F), 10) +LDWPar.ROI.y + LDWPar.ROI_initY; //+ 10;
...
void state_video_record_entry(void * para, INT32U state)
{
	...
	if (ap_LDW_get_from_config(LDW_ON_OFF) && ((ap_state_config_video_resolution_get() == 0) ||
		 (ap_state_config_video_resolution_get () == 2)))
	{
		LDWS_Enable_Flag	= 1;
		workingMemSzie		= LDWs_get_memorySize(workingImg_W, workingImg_H, &LDWPar);
		LDWs_workmem		= (INT32S *)
		gp_malloc_align(workingMemSzie, 8);
	...
	while (exit_flag == EXIT_RESUME)
	{
		...
		switch (msg_id)
		{
			case EVENT_APQ_ERR_MSG:
				audio_temp = (STAudioConfirm *)
				ApQ_para;

				if ((audio_temp->result == AUDIO_ERR_DEC_FINISH) &&
					 (audio_temp->source_type == AUDIO_SRC_TYPE_APP_RS))
				{
					//	gpio_write_io(SPEAKER_EN, DATA_LOW);
				}
				else if ((audio_temp->result == AUDIO_ERR_DEC_FINISH) &&
					 (audio_temp->source_type == AUDIO_SRC_TYPE_FS))
				{
			...
			case MSG_STORAGE_SERVICE_VID_REPLY:
				...
				if ((ap_video_record_sts_get() &VIDEO_RECORD_BUSY) == 0)
				{
					...
					STOR_SERV_FILEINFO * file_info_ptr = (STOR_SERV_FILEINFO *)
					ApQ_para;
					INT8U type			= FALSE;
