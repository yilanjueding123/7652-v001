...
INT8S ap_audio_record_init(INT32U prev_state)
{
...
#if 0
	audio_record_bg_buff = (INT32U)
	gp_malloc_align(TFT_WIDTH * TFT_HEIGHT * 2, 64);
	...
	if (logo_fd != 0xFFFF)
	{
		...
		img_info.image_source = (INT32S)
		background_image_ptr;
	...
	/*	audio_record_image_back_buff = (INT32U) gp_malloc_align(TFT_WIDTH * 20 * 2, 64);
		...
		gp_memcpy((INT8S *)audio_record_image_back_buff, (INT8S *)(audio_record_bg_buff + TFT_WIDTH * (TFT_HEIGHT - 20
		) * 2), TFT_WIDTH * 20 * 2);
	*/
...
void ap_audio_record_timer_start(void)
{
	if (audio_record_display_timerid == 0xFF)
	{
		...
		sys_set_timer((void *) msgQSend, (void *) ApQ, MSG_APQ_DISPLAY_DRAW_TIMER, audio_record_display_timerid,
			 AUDIO_RECORD_DISPLAY_TIMER_INTERVAL);
	}
}
...
void ap_audio_record_timer_draw(void)
{
	/*	if(s_usbd_pin == 0) {
			...
			if(audio_record_bg_buff && audio_record_image_back_buff) {
				gp_memcpy((INT8S *)(audio_record_bg_buff + TFT_WIDTH * (TFT_HEIGHT - 20) * 2), (INT8S *)audio_record_
		image_back_buff, TFT_WIDTH * 20 * 2);
...
INT8S ap_audio_record_reply_action(STOR_SERV_FILEINFO * file_info_ptr)
{
	...
	if (file_info_ptr)
	{
		...
		gp_memcpy((INT8S *) g_curr_audio_file_path, (INT8S *) curr_audio_file_info.file_path_addr,
			 sizeof(g_curr_audio_file_path));
		curr_audio_file_info.file_path_addr = (INT32U)
		g_curr_audio_file_path;
	}
